    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @if(isset($query))
                    <p class="mb-0">{{ $query->{$options->label} }}</p>
                @else
                    <p class="mb-0">{{ __('voyager::generic.no_results') }}</p>
                @endif

            @else

                <select
                    class="form-control @if(isset($options->taggable) && $options->taggable === 'on') select2-taggable @else select2-ajax @endif" name="{{ $options->column }}"
                    data-get-items-route="{{route('voyager.' . $dataType->slug.'.relation')}}"
                    data-get-items-field="{{$row->field}}"
                    @if(!is_null($dataTypeContent->getKey())) data-id="{{$dataTypeContent->getKey()}}" @endif
                    data-method="{{ !is_null($dataTypeContent->getKey()) ? 'edit' : 'add' }}"

                    @if (in_array('slug', app($options->model)->getFillable()))
                        data-slug="true"
                    @endif
        ...
        @elseif($options->type == 'hasMany')
            ...
            @else
                ...
                @php
                    $model = app($options->model);
                    $selected = $model::where($options->column, $dataTypeContent->{$options->key})
                        ->whereNotNull($options->column)
                        ->get();
                @endphp

                <select
                    class="form-control select2-ajax" name="{{ $row->field }}[]" multiple
                    data-get-items-route="{{ route('voyager.' . $dataType->slug.'.relation') }}"
                    data-get-items-field="{{ $row->field }}"
                    @if(!is_null($dataTypeContent->getKey())) data-id="{{$dataTypeContent->getKey()}}" @endif
                    data-method="{{ !is_null($dataTypeContent->getKey()) ? 'edit' : 'add' }}"
                >
                    ...
                    @foreach($selected as $selectedItem)
                        <option selected="selected" value="{{ $selectedItem->{$options->key} }}">
                            {{ $selectedItem->{$options->label} }}
                        </option>
                    @endforeach
