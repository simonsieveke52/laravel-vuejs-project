...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if (in_array('Kalnoy\Nestedset\NodeTrait', array_keys((new \ReflectionClass($data))->getTraits()))) {
            $rows = $rows->reject(function ($row) use ($data) {
                return in_array($row->field, [
                    $data->getParentIdName(),
                    $data->getLftName(),
                    $data->getRgtName()
                ]);
            });
        ...
        foreach ($rows->where('field', '!=', 'created_at') as $row) {
            if ($row->field === 'product_belongstomany_category_relationship') {
                continue;
            }

            // added support for hasMany relation
            if ($row->type == 'relationship' && (is_object($row->details) && $row->details->type == 'hasMany')) {
                $hasManyRelations[] = $row;
                continue;
            }
            ...
            if ($row->type == 'relationship' && ! is_object($row->details)) {
                continue;
            }
        ...
        /**
         * On each relation found with hasMany, We need to Remove
         * old items before attaching new ones, in order to
         * prevent duplicate adding, please refactor,
         * this method or make it cleaner
         */
        foreach ($hasManyRelations as $row) {
            if ($row->field == 'product_hasmany_product_image_relationship') {
                continue;
            }

            try {
                app($row->details->model)
                    ->where($row->details->column, $data->id)
                    ->update([
                        $row->details->column => null
                    ]);
            } catch (\Exception $e) {
            }

            $rows = $request->get($row->field, []);
            ...
            app($row->details->model)
                ->whereIn(app($row->details->model)->getKeyName(), $rows)
                ->update([
                    $row->details->column => $data->id
                ]);
        }
