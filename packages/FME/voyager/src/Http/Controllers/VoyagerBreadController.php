...
class VoyagerBreadController extends Controller
{
    public function index()
    {
        ...
        $tables = collect($tables)->sortByDesc('dataTypeId')->toArray();
    ...
    public function create(Request $request, $table)
    {
        ...
        try {
            $hasNestedTrait = in_array(
                NodeTrait::class,
                array_keys((new \ReflectionClass($data['model_name']))->getTraits())
            );
        } catch (\Exception $e) {
            $hasNestedTrait = false;
        }

        $data['hasNestedTrait'] = $hasNestedTrait;
    ...
    public function edit($table)
    {
        ...
        try {
            $hasNestedTrait = in_array(
                NodeTrait::class,
                array_keys((new \ReflectionClass($dataType->model_name))->getTraits())
            );
        } catch (\Exception $e) {
            $hasNestedTrait = false;
        }

        return Voyager::view('voyager::tools.bread.edit-add', compact(
            'dataType',
            'fieldOptions',
            'isModelTranslatable',
            'tables',
    ...
    public function destroy($id)
    {
        ...
        if ($res) {
            try {
                $menu = MenuItem::where('route', 'voyager.'.$dataType->slug.'.index')->firstOrFail();
                $menu->delete();
            } catch (\Exception $e) {
            }
