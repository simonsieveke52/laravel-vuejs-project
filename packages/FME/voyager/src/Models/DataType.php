...
class DataType extends Model
{
    ...
    {
        try {
            $model = $this->attributes['model_name'] ?? '';
            return trim($model) !== '' ? app($model) : null;
        } catch (Exception $e) {
            return null;
        }
    }
    ...
    public function editRowsInputs()
    {
        return $this->rows()->where('edit', 1)->where('type', '!=', 'relationship');
    }
